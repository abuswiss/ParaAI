...
const SignUpForm: React.FC<SignUpFormProps> = ({ onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await signUp({ email, password, acceptTerms });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {successMessage && (
        <div className="rounded-lg bg-green-900/20 p-4 text-sm text-green-400 border border-green-800/30 backdrop-blur-sm animate-fadeIn">
          <div className="flex">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
      ...
      <div className="flex items-start">
        <div className="flex h-5 items-center">
          <input
            ...
            className="h-4 w-4 rounded border-gray-600 bg-gray-800 text-primary focus:ring-primary focus:ring-offset-gray-900 transition-colors duration-200"
            required
          />
        </div>
        <div className="ml-3 text-sm">
          <label htmlFor="terms" className="text-gray-400">
            I agree to the <a href="#" className="text-primary hover:text-primary/80 hover:underline transition-colors duration-200">Terms and Conditions</a>
          </label>
        </div>
      </div>
