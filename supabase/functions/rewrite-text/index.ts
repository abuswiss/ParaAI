...
serve(async (req)=>{
  ...
  try {
    ...
    const { textToRewrite, instructions, surroundingContext, mode = 'improve', stream = true, userId } = payload;
    if (!textToRewrite || !userId) {
      return new Response(JSON.stringify({
        error: "Missing textToRewrite or userId"
      }), {
        headers: {
          ...corsHeaders,
